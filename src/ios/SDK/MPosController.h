...
/**
 * @return int              小于0，表示蓝牙未准备好
 * @see 消息回调函数: MPosDelegate::didGetKsnCompleted:
 * @see getJFMagcard:orderId:
 * @see getJFICData:orderId:
 */
-(NSInteger) getJFKSN;
...
/**
 * @return int              小于0，表示蓝牙未准备好
 * @see 消息回调函数: MPosDelegate::didDecodeCompleted:
 * @see openCardReader:aMount:timeOut:readType:showMsg:
 * @see getJFICData:orderId:
 */
-(NSInteger) getJFMagcard: (NSString *)transLogNo orderId: (NSString *)orderId;
 ...
 * @see getJFMagcard:orderId:
 */
-(NSInteger) getJFICData: (NSString *)transLogNo orderId: (NSString *)orderId;
...
/*! 获取KSN回调
 \param  ksn 终端号及psam号（前20字节为终端号，后16字节为psam号）
 \return 无
 */
...
/*! 刷卡数据回调
 \param  formatID 保留
 \param  encTracks 磁道信息
 \param  track1Length 一磁道长度
 \param  track2Length 二磁道长度
 \param  track3Length 三磁道长度
 \param  randomNumber 磁道随机数
 \param  maskedPAN 银行卡号明文
 \param  expiryDate 银行卡有效期明文
 \param  cardHolderName 持卡人
 \param  pinBlock 密码 24个字节密码密文(空密码则为24个F) + FF + “transLogNo”
 \param  cardType 卡类型 用于判断是否IC刷卡还是磁道刷卡，或者状态(如：降级交易)
 \param  mac  MAC值
 \param  serialNum 序列号
 ...
 \return 无
 */
- (void)didDecodeCompleted:(NSString *)formatID
        ...
        andCardHolderName:(NSString *)cardHolderName
               andPinBlock:(NSString *)pinBlock
              andCardType:(int)cardType
                   andMac:(NSString *)mac
            andcardserial:(NSString *)serialNum
               andIccData:(NSString *)data55
             andOtherInfo:(NSDictionary *)otherInfo;
 ...
 * @param data      读取的数据
 */
-(void) didDataReadResp: (MFEU_MSR_RESP)resp dataRead: (NSString *)data;
